...
export function apply(ctx) {
    ...
    const OC_CMD = `if [ -f "$HOME/.local/share/opencode/opencode.db" ]; then sqlite3 -readonly -json "$HOME/.local/share/opencode/opencode.db" "SELECT json_extract(data,'$.providerID') AS p, json_extract(data,'$.modelID') AS m, json_extract(data,'$.path.root') AS cwd, date(time_created/1000,'unixepoch','localtime') AS d, count(*) AS n, sum(coalesce(json_extract(data,'$.tokens.input'),0)) AS i, sum(coalesce(json_extract(data,'$.tokens.output'),0)) AS o, sum(coalesce(json_extract(data,'$.tokens.reasoning'),0)) AS r, sum(coalesce(json_extract(data,'$.tokens.cache.read'),0)) AS cr, sum(coalesce(json_extract(data,'$.tokens.cache.write'),0)) AS cw, round(sum(coalesce(json_extract(data,'$.cost'),0)),6) AS c, max(time_created) AS la FROM message WHERE json_extract(data,'$.role')='assistant' GROUP BY 1,2,3,4;"; fi`

    const OC_COUNT_CMD = `sqlite3 -readonly "$HOME/.local/share/opencode/opencode.db" "SELECT count(*) FROM session;" 2>/dev/null || echo 0`

    async function collectOpencode() {
      const out = await runShell(OC_CMD, 300000)
      let rows = []
      if (out.trim() !== '') {
      ...
      let sessions = 0
      try {
        const countOut = await runShell(OC_COUNT_CMD, 120000)
